...
# Read Insulin inputs
def read_insluin(fname, format_str):
    ...
    # print(st_time)
    # st_time['TimeStamp'] = pd.to_datetime(st_time['Date'].str.slice(0, 10) + ' ' + st_time['Time'], format=format_str)
    ts_df = pd.to_datetime(st_time['Date'].str.slice(0, 10) + ' ' + st_time['Time'], format=format_str).to_frame()
    ts_df.rename(columns={0: 'TimeStamp'}, inplace=True)
    st_time_df = st_time[['Index', 'BWZ Carb Input (grams)']]
    # print('ts_df:\n%s' % ts_df)
    # print('st_time_df:\n%s' % st_time_df)
    st_time = st_time_df.join(ts_df)
    # print('st_time:\n%s' % st_time)
...
def cal_meal_time(df_st, df_cgm, meal):
    ...
    if meal == 'meal':
        ...
        for y in range(len(df_nmtimes)):
            data = df_cgm.loc[(df_cgm['TimeStamp'] > df_nmtimes[y][0]) |
                              (df_cgm['TimeStamp'] < df_nmtimes[y][1])]['CGM']
            df_nm.append(data)
        # print(df_nm)
        df_nm2 = []
...
def main_fun():
    ...
    begin_time1 = read_insluin('./InsulinData.csv', '%m/%d/%Y %H:%M:%S')
    begin_time2 = read_insluin('./Insulin_patient2.csv', '%Y-%m-%d %H:%M:%S')
    ...
    df_meal_array = np.asarray(df_meal, dtype=np.int32)
    df_no_meal_array = np.asarray(df_no_meal, dtype=np.int32)
