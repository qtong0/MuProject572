...
def read_cgm(fname):
    ...
    df_cgm = df_cgm.sort_values(by=['TimeStamp'],
                                ascending=True).fillna(method='ffill')
    df_cgm = df_cgm.drop(columns=['Date',
                                  'Time', 'Index']).sort_values(by=['TimeStamp'], ascending=True)
    # print(df_cgm)
...
def read_insluin(fname):
    ...
    df_ins = df_ins.drop(columns=['Date', 'Time', 'BWZ Carb Input (grams)']).sort_values(by=['DateTime'],
                                                                                         ascending=True)
...
def cal_meal_time(df_ins,df_cgm):
    ...
    for x in range(len(df_mtime)):
        data = df_cgm.loc[(df_cgm['TimeStamp'] >= df_mtime[x][0]) &
                          (df_cgm['TimeStamp'] < df_mtime[x][1])]['CGM']
        df_m.append(data)
    df_ml_length = []
    df_mf = []
    y= 0
...
def main_fun():
    ...
    kmean_SSE = (calculateSSE(bin_1) * len(bin_1) + calculateSSE(bin_2) *
                 len(bin_2) + calculateSSE(bin_3) * len(bin_3) + calculateSSE(bin_4) * len(bin_4) +
                 calculateSSE(bin_5) * len(bin_5) + calculateSSE(bin_6) * len(bin_6)) / (total)
    ...
    dbs_SSE = (calculateSSE(bin_1) * len(bin_1) + calculateSSE(bin_2) * len(bin_2)
               + calculateSSE(bin_3) * len(bin_3) + calculateSSE(bin_4) * len(bin_4) +
               calculateSSE(bin_5) * len(bin_5) + calculateSSE(bin_6) * len(bin_6)) / (total)
    ...
    for cluster in dbs_contingency:
        cluster = cluster / float(cluster.sum()) #print("cluster #####", cluster)
        e= 0
        for x in cluster :
            if x !=0 :
                e = (cluster * [log(x, 2)]).sum()
    ...
    result = []
    result.append([kmean_SSE, dbs_SSE, kmean_entropy, dbs_entropy, kmean_purity,
                   dbs_purity])
